...
- [Links](https://github.com/typecho-fans/plugins/tree/master/Links) 插件为可选依赖。友情链接页面和首页底部友情链接需要该插件创建 `links` 数据表，初始化后即使插件停用也能直接读取数据。
...
| 首页底部链接 | 不显示 | 启用后直接读取 `links` 数据表中状态为启用且分类名为“推荐”的链接，不依赖插件输出函数。 |
...
文章编辑页的自定义字段中提供“推荐位”选项，默认不勾选。勾选“显示在首页推荐位”并发布文章后，该文章会进入首页幻灯片；取消勾选后不再显示。多篇推荐文章按发布时间从新到旧排列，草稿和未发布文章不会显示。
...
先安装并启用 Links 插件完成 `links` 数据表初始化，并确认表中存在状态为启用且分类名精确为“推荐”的链接。初始化后无需保持插件启用。
...
- 首页底部友情链接改为直接读取 `links` 数据表，不再调用 Links 插件输出函数。
- 首页底部仅显示状态为启用且分类名为“推荐”的链接。
- 移除主题设置中的推荐位文章 CID，改用文章自定义字段“推荐位”。
- 推荐文章按发布时间从新到旧展示，草稿和未发布文章自动排除。
