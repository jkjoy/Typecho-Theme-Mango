...
function getPermalinkFromCoid($coid) {
    try {
        ...
        error_log('Error in getPermalinkFromCoid: ' . $e->getMessage());
        return '';
    }
}

// 定义一个备用的_t函数，以防Typecho核心未加载
        ...
        <div class="layoutSingleColumn">
            ...
            <?php else: ?>
                ...
                try {
                    ...
                    error_log('Error in commentApprove: ' . $e->getMessage());
                ...
                <?php function threadedComments($comments, $options) {$commentClass = '';if ($comments->authorId) {if ($comments->authorId == $comments->ownerId) {$commentClass .= ' comment-by-author';} else {$commentClass .= ' comment-by-user';}}
                    // 添加错误捕获
                    try {
                        $commentApprove = commentApprove($comments, $comments->mail);
                    } catch (Exception $e) {
                        error_log('Error in threadedComments > commentApprove: ' . $e->getMessage());
                        $commentApprove = array(
                            'userDesc' => '',
                            'bgColor' => '#999',
                            'userLevel' => ''
                ...
                <article class="comment-body" id="div-<?php $comments->theId(); ?>">
                    ...
                    <div class="comment-content">
                        ...
                        // 添加错误捕获
                        try {
                            if ($comments->parent) {echo getPermalinkFromCoid($comments->parent);}
                        } catch (Exception $e) {
                            error_log('Error in displaying parent comment: ' . $e->getMessage());
                        }
                        $comments->content(); 
                        ?>
                    </div><!-- .comment-content -->
